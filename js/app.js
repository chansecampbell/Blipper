...
game.rules = function() {
    $("#left").hide();
    $("#right").hide();
    $("body").append("<h3>Blipper</h3><button class='fade-in one'>Welcome to Blipper! You are the Blip Man. Your mission is a simple one, sneak past the enemy without being detected. Enemies move in two directions so keep an eye on their move patterns and learn from your mistakes. Good luck. You may click anywhere to begin.</button>");
    $('#theme').get(0).play();
    ...
    $('body').on("keyup", function(e) {
        ...
        if (e.which === 38) {
            ...
            if ($($("li")[game.currentPosition]).hasClass("obstacle")) {
                ...
                $('#slide').get(0).play();
                setTimeout(function() {
                    $($("li")[game.currentPosition]).css("animation-name", "pulse")
                }, 1000);
                game.moveCounter++;
                game.computerMove();
            }
